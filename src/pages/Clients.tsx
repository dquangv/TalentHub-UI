...
const Clients = () => {
  ...
  const applyFilters = () => {
    ...
    if (filters.province) {
      filtered = filtered.filter(
        (client) => client.province === filters.province
      );
    }
    ...
    if (filters.title) {
      filtered = filtered.filter((client) => client.title === filters.title);
    }

    filtered = filtered.filter(
      (client) =>
        client.fromPrice >= filters.minBudget &&
        client.toPrice <= filters.maxBudget
    );
    ...
    if (filters.minRating > 0) {
      filtered = filtered.filter(
        (client) => client.averageRating >= filters.minRating
      );
    }
          ...
          {loading ? (
            ...
            <div className="col-span-3 text-center py-12 text-gray-500">
              {t("No Clients Available") ||
                "Không có nhà tuyển dụng nào phù hợp với tìm kiếm của bạn"}
            </div>
          ) : (
