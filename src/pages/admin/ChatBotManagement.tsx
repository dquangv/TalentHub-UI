...
const ChatbotManagement: React.FC = () => {
  ...
  const [unprocessedQueries, setUnprocessedQueries] = useState<
    UnprocessedQuery[]
  >([]);
  const [selectedQuery, setSelectedQuery] = useState<UnprocessedQuery | null>(
    null
  );
  ...
  const [deleteResponseDialogOpen, setDeleteResponseDialogOpen] =
    useState(false);
  ...
  const [selectedResponse, setSelectedResponse] = useState<ChatResponse | null>(
    null
  );
  ...
  const fetchUnprocessedQueries = async () => {
    try {
      ...
      notification.error({
        message: "Error",
        description: "Failed to fetch unprocessed queries",
      });
      console.error(error);
    }
  };
  ...
  const fetchIntentDetails = async (intentId: number) => {
    try {
      ...
      notification.error({
        message: "Error",
        description: "Failed to fetch intent details",
      });
      console.error(error);
      return null;
    }
  ...
  const handleAddIntent = async () => {
    try {
      ...
      notification.success({
        message: "Thành công!",
        description: "Thêm mới ý định thành công!",
      });
      fetchIntents();
      ...
      notification.error({
        message: "Error",
        description: "Failed to add intent",
      });
      console.error(error);
    }
  };
  ...
  const handleUpdateIntent = async (intentId: number) => {
    ...
    try {
      const intentDTO: Partial<IntentDTO> = {
        ...
        trainingPhrases:
          selectedIntent.trainingPhrases?.map((p) => p.text) || [],
      };

      await chatbotService.updateIntent(intentId, intentDTO);
      notification.success({
        message: "Thành công!",
        description: "Cập nhật ý định thành công!",
      });
      fetchIntents();
  ...
  const handleDeleteIntent = async () => {
    ...
    try {
      ...
      notification.success({
        message: "Thành công!",
        description: "Xóa ý định thành công!",
      });
      fetchIntents();
  ...
  const handleAddResponse = async () => {
    ...
    try {
      await chatbotService.addResponse(selectedIntent.id, {
        ...editResponse,
        responseText: editResponse.text,
        text: undefined,
      });
      notification.success({
        message: "Thành công!",
      ...
      notification.error({
        message: "Error",
        description: "Failed to Thêm phản hồi",
      });
      console.error(error);
    }
  };
  ...
  const handleUpdateResponse = async () => {
    ...
    try {
      await chatbotService.updateResponse(selectedResponse.id, {
        ...editResponse,
        responseText: editResponse.text,
        text: undefined,
      });
    ...
    <div className="space-y-4">
                    ...
                    {chatHistory.length === 0 ? (
                      ...
                      chatHistory.map((chat, index) => (
                        <div
                          key={index}
                          className={`flex ${
                            chat.isBot ? "justify-start" : "justify-end"
                          }`}
                        >
                          <div
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
            <Card className="md:col-span-2">
              <CardHeader>
                ...
                <CardDescription>
                  Đây là các câu hỏi của người dùng chưa được xử lý với các ý
                  định nghiệp vụ.
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            {selectedQuery && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Xử lý câu hỏi này để có thể trả lời vào những lần sau
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      <Label>Câu hỏi của người dùng</Label>
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="ignored_query">
                            Bỏ qua câu hỏi này
                          </SelectItem>
                          {intents.map((intent) => (
                            <SelectItem
                    ...
                    {processingData.intentName &&
                      processingData.intentName !== "ignored_query" && (
                        <>
                          {processingData.intentName === "new_intent" && (
                            <div className="space-y-2">
                  ...
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <div className="flex space-x-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setEditIntentDialogOpen(true)}
                      >
                      ...
                      <Button
                        variant="destructive"
                        size="sm"
                        onClick={() => setDeleteIntentDialogOpen(true)}
                      >
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      <div>
                        ...
                        <div className="space-y-2">
                          {selectedIntent.trainingPhrases &&
                          selectedIntent.trainingPhrases.length > 0 ? (
                            selectedIntent.trainingPhrases.map(
                              (phrase, index) => (
                                <div
                                  key={index}
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          ...
                          selectedIntent.responses.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full py-12 text-center">
                  <div className="space-y-2">
                    ...
                    <h3 className="text-lg font-medium">
                      Chưa chọn ý định nào
                    </h3>
                    <p className="text-sm text-muted-foreground">
                      Chọn một ý định từ danh sách hoặc tạo một cái mới để xem
                      chi tiết của nó.
                    </p>
                  </div>
                </div>
        ...
        <TabsContent value="statistics">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                  Tổng số câu hỏi chưa được xử lý
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">
                  {stats?.unrecognizedQueries || 0}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="md:col-span-2 xl:col-span-4">
              <CardHeader>
                <CardTitle>Các ý định được quan tâm nhiều nhất</CardTitle>
                <CardDescription>
                  Các ý định được phát hiện thường xuyên nhất
                </CardDescription>
              </CardHeader>
              <CardContent>
                {stats?.topIntents && stats.topIntents.length > 0 ? (
                          ...
                          <div
                            ...
                            style={{
                              width: `${
                                (item.count /
                                  Math.max(
                                    ...stats.topIntents.map((i) => i.count)
                                  )) *
                                100
                              }%`,
                            }}
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    No data available. Start using the chatbot to generate
                    statistics.
                  </div>
                )}
              </CardContent>
      ...
      <Dialog open={intentDialogOpen} onOpenChange={setIntentDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Tạo ý định mới</DialogTitle>
            <DialogDescription>
              Tạo ý định mới với các cụm từ huấn luyện và các phản hồi.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="intentName">Tên ý định</Label>
              <Input
                id="intentName"
                value={newIntent.intentName}
                onChange={(e) =>
                  setNewIntent({ ...newIntent, intentName: e.target.value })
                }
                placeholder="Enter intent name"
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="intentDescription">Mô tả (không bắt buộc)</Label>
              <Textarea
                id="intentDescription"
                value={newIntent.description}
                onChange={(e) =>
                  setNewIntent({ ...newIntent, description: e.target.value })
                }
                placeholder="Enter intent description"
              />
            </div>
            <div className="space-y-2">
              <Label>Cụm từ huấn luyện</Label>
              {newIntent.trainingPhrases.map((phrase, index) => (
                <div key={index} className="flex space-x-2">
                  <Input
                    ...
                    placeholder={`Cụm từ huấn luyện ${index + 1}`}
                  />
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => {
                      const updated = newIntent.trainingPhrases.filter(
                        (_, i) => i !== index
                      );
                      setNewIntent({ ...newIntent, trainingPhrases: updated });
                    }}
              ...
              {newIntent.responses.map((response, index) => (
                <div key={index} className="flex space-x-2">
                  <Textarea
                    ...
                    placeholder={`Phản hồi ${index + 1}`}
                  />
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => {
                      const updated = newIntent.responses.filter(
                        (_, i) => i !== index
                      );
                      setNewIntent({ ...newIntent, responses: updated });
                    }}
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setNewIntent({
                      ...newIntent,
                      requiresDbQuery: checked as boolean,
                    })
                  }
                />
              ...
              {newIntent.requiresDbQuery && (
                <Textarea
                  ...
                  onChange={(e) =>
                    setNewIntent({ ...newIntent, dbQuery: e.target.value })
                  }
          ...
          <DialogFooter className="sm:justify-between">
            ...
            <Button
              ...
              disabled={
                ...
                newIntent.trainingPhrases.some((p) => !p) ||
                newIntent.responses.some((r) => !r)
              }
            >
              Tạo ý định
            </Button>
      ...
      {/* Edit Intent Dialog */}
      <Dialog
        open={editIntentDialogOpen}
        onOpenChange={setEditIntentDialogOpen}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Chỉnh sửa ý định</DialogTitle>
            <DialogDescription>
              Cập nhật chi tiết ý định và các cụm từ đào tạo.
            </DialogDescription>
          </DialogHeader>
          {selectedIntent && (
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="editIntentName">Tên ý định</Label>
                <Input
                  id="editIntentName"
              ...
              <div className="space-y-2">
                <Label htmlFor="editIntentDescription">Mô Tả</Label>
                <Textarea
                  ...
                  selectedIntent.trainingPhrases.map((phrase, index) => (
                    <div key={index} className="flex space-x-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const updated =
                            selectedIntent.trainingPhrases!.filter(
                              (_, i) => i !== index
                            );
                          setSelectedIntent({
                            ...selectedIntent,
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setEditIntentDialogOpen(false)}
            >
              Đóng
            </Button>
            <Button
              onClick={() =>
                selectedIntent && handleUpdateIntent(selectedIntent.id)
              }
      ...
      <AlertDialog
        open={deleteIntentDialogOpen}
        onOpenChange={setDeleteIntentDialogOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete the intent "
              {selectedIntent?.intentName}" and all associated training phrases
              and responses.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {/* Thêm phản hồi Dialog */}
      <Dialog open={responseDialogOpen} onOpenChange={setResponseDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Thêm phản hồi</DialogTitle>
            <DialogDescription>
              Thêm phản hồi mới cho ý định "{selectedIntent?.intentName}".
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setEditResponse({ ...editResponse, text: e.target.value })
                }
      ...
      <Dialog
        open={editResponseDialogOpen}
        onOpenChange={setEditResponseDialogOpen}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Chỉnh sửa phản hồi</DialogTitle>
            <DialogDescription>
              Cập nhật phản hồi cho ý định "{selectedIntent?.intentName}".
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="edittext">Nội dung phản hồi</Label>
              <Textarea
                id="edittext"
                value={editResponse.text}
                onChange={(e) =>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setEditResponseDialogOpen(false)}
            >
              Cancel
            </Button>
            <Button
