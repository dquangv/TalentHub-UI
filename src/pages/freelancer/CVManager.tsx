  ...
  const handleDeleteCV = async (cvId: number) => {
    ...
    if (cv?.jobs && cv.jobs.length > 0) {
      notification.error({
        ...
        description:
          "CV này đã được sử dụng để ứng tuyển công việc và không thể xóa.",
      });
      return;
    }
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <Button
                      ...
                      title={
                        cv.jobs && cv.jobs.length > 0
                          ? "Không thể xóa CV đã được sử dụng để ứng tuyển"
                          : ""
                      }
                    >
                      {deletingIds.includes(cv.id) ? (
      ...
      {cvs.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-6 max-w-md mx-auto">
            Bạn chưa có CV nào trong hệ thống. Hãy tải lên CV đầu tiên để bắt
            đầu.
          </p>
        </div>
      )}
