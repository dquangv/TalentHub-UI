...
const AddressSelector: React.FC<AddressSelectorProps> = ({
            ...
            if (value && !initialLoadComplete) {
                ...
                try {
                    ...
                    if (parsedAddress.provinceCode) {
                        const province = provinces.find(p => p.code.toString() === parsedAddress.provinceCode) || null;
                            ...
                            if (provinceDetails && provinceDetails.districts) {
                                ...
                                if (parsedAddress.districtCode) {
                                    ...
                                    if (district) {
                                        ...
                                        const districtDetails = await addressService.getDistrictDetails(district.code.toString());
                                        if (districtDetails && districtDetails.wards) {
                                            setWards(districtDetails.wards);

                                            // If we have ward code
                                            if (parsedAddress.wardCode) {
                                                const ward = districtDetails.wards.find(
    ...
    const handleProvinceChange = async (provinceCode: string) => {
        ...
        try {
            const province = provinces.find(p => p.code.toString() === provinceCode) || null;
            ...
            if (province) {
                const provinceDetails = await addressService.getProvinceDetails(provinceCode);
    ...
    const handleDistrictChange = async (districtCode: string) => {
        ...
        try {
            const district = districts.find(d => d.code.toString() === districtCode) || null;
            ...
            if (district) {
                const districtDetails = await addressService.getDistrictDetails(districtCode);
                ...
                <div className="space-y-3 border rounded-md p-3">
                    ...
                    <div className="space-y-3">
                        ...
                        <div>
                            ...
                            <Select
                                value={addressData.province ? addressData.province.code.toString() : ''}
                            ...
                            {isLoadingProvinces && (
                                <div className="text-xs text-muted-foreground mt-1">Đang tải danh sách tỉnh/thành phố...</div>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            <Select
                                value={addressData.district ? addressData.district.code.toString() : ''}
                            ...
                            {isLoadingDistricts && (
                                <div className="text-xs text-muted-foreground mt-1">Đang tải danh sách quận/huyện...</div>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            >
                                ...
                                <SelectContent className="max-h-60">
                                    ...
                                    {isLoadingWards ? (
                                        ...
                                        <div className="p-2 text-center text-muted-foreground">
                                            {addressData.district ? 'Không có phường/xã' : 'Vui lòng chọn quận/huyện trước'}
                                        </div>
                            ...
                            {isLoadingWards && (
                                <div className="text-xs text-muted-foreground mt-1">Đang tải danh sách phường/xã...</div>
                            )}
                        </div>
                    </div>

                    <div className="pt-2">
                        <div className="text-xs text-muted-foreground mb-1">Địa chỉ đầy đủ:</div>
                        <div className="text-sm p-2 bg-muted rounded-md">
                            {value || "Chưa có địa chỉ"}
                        </div>

                        {(loading || isLoadingProvinces || isLoadingDistricts || isLoadingWards) && renderLoadingSpinner()}
                    </div>
                </div>
            )}
        </div>
