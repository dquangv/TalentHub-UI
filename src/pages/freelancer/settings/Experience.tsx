...
const ExperienceComponent = () => {
  ...
  const fetchExperiences = async () => {
    try {
      ...
      const response = await experienceService.getFreelancerExperiences(
        freelancerId
      );
      ...
      notification.error({
        message: "Lỗi",
        description:
          "Không thể tải danh sách kinh nghiệm làm việc. Vui lòng thử lại sau.",
      });
    } finally {
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        setOriginalExperiences(
          originalExperiences.filter((exp) => exp.id !== id)
        );
      ...
      notification.error({
        message: "Lỗi",
        description:
          "Không thể xóa kinh nghiệm làm việc. Vui lòng thử lại sau.",
      });
    } finally {
  ...
  const updateExperienceField = (
    ...
    if (
      value === "" &&
      ["companyName", "position", "startDate"].includes(field)
    ) {
  ...
  const saveExperience = async (id: number) => {
    ...
    if (
      ...
      notification.error({
        message: "Lỗi",
        description:
          "Vui lòng điền đầy đủ thông tin bắt buộc: Tên công ty, Vị trí, Ngày bắt đầu.",
      });
      return;
    }
    ...
    try {
      ...
      notification.error({
        message: "Lỗi",
        description:
          "Không thể lưu kinh nghiệm làm việc. Vui lòng thử lại sau.",
      });
    } finally {
      setSavingIds((prev) => prev.filter((itemId) => itemId !== id));
    }
      ...
      {experiences.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-6 max-w-md mx-auto">
            Bạn chưa có thông tin kinh nghiệm làm việc nào. Hãy thêm kinh nghiệm
            làm việc để nhà tuyển dụng có thể đánh giá tốt hơn.
          </p>
        </div>
      )}
              ...
              <div className="flex items-start justify-between mb-6">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      experience.id && removeExperience(experience.id)
                    }
                    disabled={isSaving}
                  >
                  ...
                  <div className="space-y-2">
                    {!currentlyWorking[experience.id || 0] && (
                      <Input
                        ...
                        disabled={
                          isSaving || currentlyWorking[experience.id || 0]
                        }
                      />
                    )}

                    {currentlyWorking[experience.id || 0] && (
                      <Input type="text" value="Hiện tại" disabled={true} />
                    )}
                  </div>
                </div>
